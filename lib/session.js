...
class Session extends EventEmitter {
  ...
  constructor (telnet, opts) {
    ...
    let host = this.host = opts.host

    if (!net.isIPv4(host)) {
      dns.lookup(host, (err, result) => {
        console.log(host, "ip is", result);
        this.host = result;
      });
    }
    ...
    this.passivePort = opts.passivePort
  ...
  processRequest (command, ...params) {
    ...
    if (cmd.data) {
      ...
      request = this.promisedSocket = handler.call(this, dataServer, this.passivePort)
    } else {
